...
ARM_ACTION_DECISION_KR = """이미지 속 사람의 행동이나 제스처를 분석하여 가장 적절한 로봇 팔 행동을 **하나만** 결정하세요.

사람의 제스처, 자세, 로봇과의 근접성(기준: {depth_m}m)을 정밀하게 고려하세요.

**[로봇이 할 수 있는 행동]**

* **키스 제스처:**
    * `two-hand kiss`: 얼굴이나 입술에 두 손을 모아서 입맞춤을 준비 할 때
    * `left kiss`: 얼굴이나 입술에  왼손을 모아서 입맞춤을 준비 할 때
    * `right kiss`: 얼굴이나 입술에 오른손을 모아서 입맞춤을 준비 할 때
* **기본 동작:**
    * `hands up`: 사람이 두 손을 머리 위로 들 때 ('손들어' 자세)
    * `clap`: 사람이 박수를 칠 때
    * `high five`: 사람이 하이파이브를 위해 손바닥을 들 때
    * `hug`: 사람이 두 팔을 벌려 포옹 자세를 취할 때
* **하트 제스처:**
    * `heart`: 사람이 두 손으로 큰 하트를 만들 때 또는 손 하트가 보일 때
    * `right heart`: 사람이 한 손으로 작은 하트(K-하트)를 만들 때
* **의사소통:**
    * `reject`: 사람이 손바닥을 앞으로 밀며 '거부', '정지', '아니' 의사를 표할 때
* **웨이브 (손 흔들기):**
    * `x-ray`: 사람이 두 손바닥을 앞으로 내밀어 스캔하는 듯한 포즈를 취할 때
    * `face wave`: 사람이 얼굴 가까이에서 손을 흔들 때 (안녕)
    * `wave`: 사람이 어깨 높이나 머리 위로 손을 흔들 때 (일반적인 손 흔들기)
    * `shake`: 사람이 악수를 청하기 위해 (주로 오른손을) 앞으로 내밀 때

**[매우 중요한 규칙]**

1.  이미지에 사람이 없거나
2.  사람이 있지만 위에 정의된 **특별한 상호작용 제스처를 취하지 않거나**
3.  상호작용하기에 너무 멀리(예: 2.5m 이상) 있거나 적절하지 않은 상황일 경우

반드시 **"NO_ACTION"**이라고 응답하세요.

응답은 다음 목록에서 정확히 하나의 행동 이름만 선택해야 합니다:
[two-hand kiss, left kiss, right kiss, hands up, clap, high five, hug, heart, right heart, reject, x-ray, face wave, wave, shake, NO_ACTION]
"""
